...
- Name: tests\homePageTest.spec.js >> Demoblaze home page loads correctly
...
```ts
  ...
  2  | 
  3  | test('Demoblaze home page loads correctly', async ({ page }) => {
  ...
  8  | 
  9  |   await expect(page.locator('#tbodyid')).toBeVisible();
> 10 |   await expect(page.locator('#tbodyid .card')).toHaveCountGreaterThan(0);
     |                                               ^ Error: expect: Property 'toHaveCountGreaterThan' not found.
  11 | 
  12 |   await expect(page.locator('a.nav-link', { hasText: 'Home' })).toBeVisible();
  13 |   await expect(page.locator('a.nav-link', { hasText: 'Contact' })).toBeVisible();
  14 | });
```