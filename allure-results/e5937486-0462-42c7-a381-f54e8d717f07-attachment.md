...
- Name: tests\softAssertion.spec.js >> Assertions in Playwright
...
Expected: "noCommerce demo store. Register"
Received: "nopCommerce demo store. Register"
Timeout:  5000ms

Call log:
  - Expect "soft toHaveTitle" with timeout 5000ms
    9 × unexpected value "nopCommerce demo store. Register"
...
```ts
  ...
  2  | 
  3  | test('Assertions in Playwright',async({page})=>{    
  4  | 
  5  |      //open app url
  6  | 
  7  |     await page.goto('https://demo.nopcommerce.com/register?returnUrl=%2Felectronics');
  8  | 
  9  |     //Ensures the page is navigated to the given URL.
  10 |     await expect.soft (page).toHaveURL('https://demo.nopcommerce.com/register?returnUrl=%2Felectronics');
  11 |  
  12 |     //Ensures the page has the given title. change the title to fail the test and see the result
> 13 |     await expect.soft (page).toHaveTitle('noCommerce demo store. Register');
     |                              ^ Error: expect(page).toHaveTitle(expected) failed
  14 | 
  15 | });
  16 | 
